...
df=pd.read_parquet("../0.Data_after_cleaning/cleaned_aviation_data_v1_INVALID.parquet")
...
df.loc[df["arrival_country"].isin(updated_country_to_continent.keys()), "arrival_continent"] = df["arrival_country"].map(updated_country_to_continent)

df.to_parquet("../0.Data_after_cleaning/cleaned_aviation_data_v2_INVALID.parquet")